...
| `reversibleLocalWrite` | 写入 Talk 启动时锁定的输入框 | 用户明确要求写入后自动执行；目标复验、Tool Call 幂等和系统撤销必须同时成立 |
...
进程内锁定一个非密码输入目标；Realtime 只有在用户明确要求“写入、放入
或填入”时才能调用 `write_focused_input` 并给出完整最终文字。客户端把它
转换为 `reversibleLocalWrite + automaticWhenTargetLocked` ActionProposal，
`FocusedInputActionExecutor` 校验策略、复验原目标并返回 `ActionReceipt`，
不显示第二次确认。它使用会话内 `WorkID` 维持 Proposal 与 Receipt 的关联，
但当前没有创建持久化 `WorkRecord`，也不能跨 Talk 恢复。因此它是
`foreground action work scope`，不是已经完成的 Work 权限账本。会话结束会
释放 AX 句柄；完整写入正文、输入框内容和句柄不进入 Backend 或诊断日志。

动作工具回执不能制造并发 Realtime Response：用户正在
...
这条动作在没有 final ASR 时仍可执行，这是明确的产品取舍：Realtime 对
最新语音的理解被用作写入意图来源，但自动策略只覆盖锁定目标上的可撤销
本地写入，并通过目标复验、单动作串行和 Tool Call ID 幂等限制误操作范围。
后台 Work、外部副作用、跨会话历史和长期记忆仍必须等待可靠 final ASR
或独立确认，不能复用这条低风险例外。
...
- Realtime Talk 已能调用 `write_focused_input`；客户端只把它转换为 `reversibleLocalWrite + automaticWhenTargetLocked` ActionProposal，并按 Tool Call ID 缓存结果、串行执行本地写入。
- `FocusedInputActionExecutor` 校验自动策略，只复验并写入 Talk 启动时锁定的非密码 Accessibility 目标，使用现有可撤销剪贴板路径，并返回 `succeeded / failed / unknown` ActionReceipt；成功不展开界面，失败或未知时给出轻量提示。
...
- Backend 已包含开发期 Mock Work Runtime，但没有持久化 Store、真实 Agent Adapter、外部副作用 Permission Runtime 或可恢复的 Delivery Ledger。
...
- 持久化 Work Permission 与 Result Delivery 账本；当前只有会话内前台输入框 Action、低风险自动执行策略和即时回执。
- `allowOnce / allowForSession / reject / cancelled`、外部副作用与破坏性动作的权限策略；当前尚未实现高风险动作确认 Runtime。
...
- 本地执行一个可撤销写入动作。
- 为可撤销本地写入完成自动策略、Tool Call 幂等和目标失效检查；为未来外部副作用保留 `allowOnce/allowForSession/reject` 目标设计。
- 状态：第一条 `writeFocusedInput` ActionProposal、本地目标锁定、自动策略校验、目标 revision 复验、幂等执行和 ActionReceipt 已完成定向自动检查。
- 待完成：真实 Mac 跨应用验收、持久化 Work/Permission/Delivery 关联和高风险确认；发送、提交、文件、浏览器与坐标动作仍未开放。
