...
- Computer Use 首个纵向切片已落地：`ComputerUseRuntime` 提供能力目录、观察、动作效果和独立验证契约；`NativeComputerUseRuntime` 通过原生 AppKit、CGEvent、TextEdit Automation 完成固定任务，`TextEditSmokeTaskRunner` 在每一步后重新观察，并且只有 UTF-8 文件精确读回后才返回成功。该适配器不是通用 Planner，也不代表 Cua 已集成。
...
- 高风险 Permission Runtime、完整 Capability Router、真实远端 Action RPC Transport，以及输入框写入之外的通用窗口/AX/截图执行器；当前只有受限 TextEdit 烟囱适配器。
- Work 权限和结果交付账本。
...
- 以 Provider-neutral Adapter 验证 Cua Driver 候选或等价 Runtime；当前已先落地原生 Swift 等价适配器，Cua 仍待签名和分发可行性验证。
- 现有固定 TextEdit 任务入口已打通原生应用激活、快捷键、文字输入、保存和文件读回；下一步再扩展为文字任务台，覆盖应用/窗口观察、AX/截图、点击、输入、滚动和快捷键。
