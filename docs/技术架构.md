...
> 版本：0.6  
> 日期：2026-08-02  
> 状态：目标架构与渐进迁移方案。除“当前实现”明确列出的内容外，不代表代码已经实现。
...
当前 `ConversationAudioServicing` 接口保留。接下来先做技术验证，再决定是否更换实现：

```text
ConversationAudioServicing
├── AVAudioEngineVoiceProcessingService (当前)
├── VoiceProcessingIOService (技术验证)
└── MockConversationAudioService
```

VoiceProcessingIO 验收不是“能出声”，而是：

- Friday 完整播放一句话时不会被自身扬声器打断；
- 用户中途说话能在自然时间内打断；
- 回复结束后用户立即接话不会被尾音门错误吞掉；
- 耳机、内置扬声器和设备切换有明确行为；
- 播放开始/结束回执来自真实音频消费，而不是仅靠服务端事件。

若技术验证不优于当前实现，则保留现有服务，不因参考项目而强行替换。
...
- `FridayApp.swift` 约 1,014 行，`AppState` 同时承担组合根、权限、健康检查、Dictate 编排、Talk 互斥和 UI 回调。
- `ConversationCoordinator.swift` 约 746 行，同时承担音频缓冲、会话状态、VAD 事件、插话、成本、超时、问候、屏幕附件和浮层映射。
- `InputOverlay.swift` 约 838 行，窗口生命周期、布局和多种产品状态继续集中。
- 身份链已进入 Runtime，但尚未通过 ConversationSync 写入 Conversation Store，也尚未关联 final ASR 和 Work。
- Backend 目前只是凭证、健康和预算服务，没有 Work、Agent、Permission 或 Delivery Runtime。
...
- `UserTurnTranscriptionProvider`、ConversationBootstrap 和重连去重。
- Work 状态机、调度、持久化和去重。
- 稳定协调 Agent Session。
- Agent Backend / ACP Adapter。
- ActionProposal 与本地 ActionExecutor 契约。
- Work 权限和结果交付账本。
- VoiceProcessingIO 对照验证。
...
- 待完成：真实播放开始回执；播放结束目前已由音频缓冲实际消费完成触发。
