...
- `ActionProposal`、`ActionReceipt` 和带版本、方法校验及 64KB 上限的 RPC 编解码已落地；目前只传输数据，不批准或执行任何本地动作。
...
- 本地 ActionExecutor、目标复验、Permission Runtime 和真实 Action RPC Transport；ActionProposal/Receipt 目前只有中立数据契约。
- Work 权限和结果交付账本。
