...
可撤销文字写入是更短的同步 Action 路径，不创建后台工单：
...
-> RunningApplicationResolver（运行列表、Bundle ID、有限别名）
-> FocusedInputActionExecutor（目标复验、粘贴、撤销证据）
-> ActionReceipt
-> Realtime tool output
```

显式 `application` 优先于 Talk 启动时锁定的输入目标；没有显式应用时才使用锁定目标。解析失败、多个同名候选、密码框、无聚焦可编辑控件或焦点复验失败时，执行器返回失败且不退回其他应用。`dispatched` 但无法读取目标值时返回 `unknown`，不能对用户声称已经完成。
...
当前过渡实现尚无通用 `CapabilityRegistry`，但第一条 `write_focused_input` 已遵守同一信任边界：模型只能提供文字和用户说出的应用名；运行应用身份、Accessibility 目标、执行结果和撤销证据都由 Friday.app 在本地确定。
...
候选 Runtime 可以自己实现这个端口。LiveKit Stage 0 的设计由同一个 Runtime/Transport 负责输入音量、输出音量、播放完成和插话确认，因此不会同时启动 Friday 的 `ConversationAudioService`。真实 SDK 尚未接入，这里只确立所有权约束，不代表 LiveKit AEC 已验证。
...
上行音频还要经过本地 `ConversationInputGate`：普通收听约 100ms 连续近场声音才放行；Friday 播放时提高阈值，强近场且具有语音起伏的声音约 260ms 可确认，普通近场候选约 380ms 可确认；确认后最多补回约 450ms 预录。输入门检测到约 550ms 静音或稳定背景尾音后，会继续向 Realtime 发送最多四秒零值 PCM，而不是立即停止网络音频；用户恢复有效语音时取消静音尾并补回预录，Provider 仍未形成端点时则退出本轮并提示重试。半双工降级在播放期间不上传任何麦克风音频。

Realtime 会话使用 `near_field` 降噪和 high-eagerness semantic VAD，同时关闭服务端 `create_response` 与 `interrupt_response`。按照 Realtime WebSocket 的 VAD 流程，客户端持续使用 `input_audio_buffer.append` 提交语音及端点静音，不在 VAD 开启时手工 `commit`。服务端 `speech_stopped` 只表示一个候选端点；客户端保留 450ms 续句窗口，期间恢复说话会取消本次回复计划，窗口结束后才显式发送一次 `response.create`。这样端点可以尽快到达，同时正常短停顿不会立刻触发抢答。
...
- 本地停说后最多四秒零值 PCM 会继续送达 Provider；恢复语音会取消静音尾，端点仍未形成时不会无限停在用户说话态。
...
- `ConversationActionBridge` 已把 Realtime `write_focused_input` 转换为 ActionProposal，并按 Tool Call ID 缓存结果，避免同一调用重复写入。
- `RunningApplicationResolver` 已支持运行应用名、Bundle ID、有限别名和歧义拒绝；当前开发机上的 “Codex” 映射到 `com.openai.codex`，即使显示名是 ChatGPT。
- `FocusedInputActionExecutor` 已通过显式应用或会话锁定目标复验输入框，并把真实粘贴结果映射为 `succeeded/failed/unknown`。只有 `reversibleLocalWrite + permission.none` 能自动执行。
...
- 状态：`ConversationRuntimeSession` 已统一 Provider、音频端口、所有权描述和原子启停；当前产品由 Direct Realtime 包装器保持原行为。
- 状态：LiveKit 候选已完成无 SDK、无网络、无费用的 Stage 0 假 Transport 契约；是否进入真实语音 Stage 1 需要单独授权和预算。
...
- Agent 只输出结构化 ActionProposal；Proposal/Receipt 和版本化 RPC 数据契约已完成。
- 本地可撤销输入框写入、应用名解析、目标失效检查和回执已完成代码与零费用测试，待真实语音、Codex 输入框和 `Command-Z` 验收。
- 完成高风险 `allowOnce/allowForSession/reject`、Capability Registry 和远端 Action Transport。
