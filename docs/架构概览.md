...
> 版本：0.8
> 日期：2026-08-03
> 状态：目标架构。本文用于快速理解系统；已经实现和尚未实现的边界见第 10 节。

## 1. 一句话说明

Friday 是一个原生 macOS AI 助手：它用低延迟语音和用户交流，理解用户主动提供的电脑上下文，并把需要工具或等待的复杂请求交给后台 Agent 执行。

用户只面对一个 Friday，但系统内部不是“一个模型包办一切”，而是三部分协作：
...
界面如何承载这些 Runtime，而不把内部复杂度暴露给用户，见[《交互架构》](交互架构.md)。灵动岛只负责当下语音，后台 Work、历史与 Memory 进入主窗口。
...
Agent 适合需要工具、等待、跨应用、多步骤或有外部副作用的请求。它不应该拖慢普通听写和简单问答。
...
Agent 不能直接操作 AppKit、Accessibility 或鼠标。它只能产生结构化 `ActionProposal`，由 Mac 客户端检查目标、风险和执行策略后处理。

| 风险 | 示例 | 默认策略 |
| --- | --- | --- |
| 只读 | 读取用户主动框选的区域 | 本次触发即授权 |
| 可撤销本地写入 | 写入 Talk 启动时锁定的输入框 | 明确写入意图后自动执行；必须可撤销、复验目标并按 Tool Call 幂等 |
| 外部副作用 | 发消息、发邮件、提交表单 | 展示完整预览并逐次确认 |
| 破坏性动作 | 删除、购买、修改系统权限 | 逐次确认，必要时二次确认 |

取消不是隐藏 UI。Friday 必须等待执行端确认停止，再把 Work 标记为 `cancelled`。

当前 Alpha 先落地了最小的前台动作闭环：用户启动 Talk 时，Friday 只在
本机内存锁定一个非密码输入目标；模型只有在用户明确要求写入时才能给出
完整最终文字；客户端把它限制为自动可撤销策略，复验原目标、串行执行并
返回 `ActionReceipt`，不再展示二次确认。这条路径带会话内 Work identity
以关联动作和回执，但还没有持久化 `WorkRecord`，会话结束即释放 AX 句柄。
因此它证明了受限动作闭环，不代表后台 Work、恢复和高风险权限账本完成。
...
- Realtime Dictate 与全双工 Talk；
- 用户主动区域框选和短期图片上下文；
- Provider 与 Mock 边界；
- Talk Session 身份、token 和费用账本；
- Provider 临时 ID 到 Friday 自有 Turn、Response、Assistant Item 和 Playback ID 的关联器，能够拒绝上一轮迟到音频。
- 内存 Mock Work Runtime、WorkDraft 来源/独立确认边界，以及不阻塞 Talk 的结果交付主干；
- 第一条 `writeFocusedInput` ActionProposal、自动可撤销策略、Tool Call 幂等、本地目标复验和 ActionReceipt；发送与提交仍不支持。
...
- 持久化 Work Scheduler、Store 和重启恢复；当前只有内存 Mock Work；
- Agent Adapter 与稳定协调 Session；
- 持久化 Permission/Delivery Ledger、`allowOnce/allowForSession` 和更多风险等级；当前只有会话内输入框写入的低风险自动策略；
- Agent 任务界面。
...
```text
1. 继续拆分现有 Talk Runtime（身份链与 Presentation 已完成，音频生命周期仍待拆分）
2. 建立可靠 final ASR、稳定 Thread 和 Conversation Store
3. 建立 ContextSnapshot、ConversationBootstrap 和 Context Assembler
4. 加入滚动摘要和可确认的最小 Memory Store
5. 使用 Mock Agent 打通最小 Work Runtime（内存主干已完成）
6. 加入 ActionProposal、风险策略和一个可撤销写入动作（代码已完成，待真实 Mac 验收与持久化）
7. 接入单一真实 Agent Backend
8. 加入后台结果交付和任务管理界面
9. 有证据需要时再扩展 ACP、多 Agent 和向量检索
```
