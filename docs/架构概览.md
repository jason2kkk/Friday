...
- `ActionProposal`、`ActionReceipt` 与 64KB 上限的版本化 RPC 数据契约已存在，但没有本地执行器、权限决策或远端执行能力。
...
- Action Permission Runtime、本地 ActionExecutor 和真实动作传输；
...
6. 在已定义的 ActionProposal/Receipt 契约上加入权限和一个可撤销写入动作
