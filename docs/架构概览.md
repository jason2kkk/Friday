...
当前第一条真实动作是语音 Agent 的可撤销文字写入。它不需要先创建后台 Work：Realtime 只解释“写什么、写到哪个应用”，Friday.app 才解析本机应用、复验输入框并执行。复杂多步骤请求仍进入独立 Work Runtime。

```text
用户：“把 1、2、3、4 写入 Codex”
-> Realtime: write_focused_input(text, application="Codex")
-> ConversationActionBridge: ActionProposal
-> RunningApplicationResolver: com.openai.codex
...
- `ActionProposal`、`ActionReceipt` 与 64KB 上限的版本化 RPC 数据契约已存在；当前只有可撤销输入框写入具备本地执行器，尚无高风险权限决策或远端执行能力。
- `ConversationActionBridge`、运行中应用解析器和输入框动作执行器已接通第一条可撤销本地写入；显式应用名优先于 Talk 启动时锁定目标，Codex 通过 Bundle ID 映射到实际运行应用。
- 写入工具按 Tool Call ID 幂等，只在本地回执为 `succeeded` 时允许模型明确声称完成；这条路径不发送、提交或发布内容。
...
- 高风险 Action Permission Runtime、远端 Action Transport 和除输入框写入之外的执行器；
...
```text
1. Talk Runtime 生命周期与音频所有权边界（已完成 Stage 0；产品仍使用 Direct Realtime）
...
5. 使用 Mock Agent 打通最小 Work Runtime
6. 在已定义的 ActionProposal/Receipt 契约上加入权限和一个可撤销写入动作（第一条本地写入代码已完成，待真机验收）
