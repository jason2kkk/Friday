...
| 可撤销本地写入 | 写入当前输入框 | 当前 Alpha 也要展示完整预览并逐次确认 |
...
本机内存锁定一个非密码输入目标；模型可以提出完整文字，但不能执行；
灵动岛展示预览后，只有一次按钮许可可以触发本地写入，执行器再返回
`ActionReceipt`。这条路径带会话内 Work identity 以关联动作和回执，但还
没有持久化 `WorkRecord`，会话结束即取消待确认许可并释放 AX 句柄。因此
它证明了权限和执行边界，不代表后台 Work、恢复和任务账本已经完成。
...
- 第一条 `writeFocusedInput` ActionProposal、一次性视觉 Permission、本地目标复验和 ActionReceipt；确认前不会写入，发送与提交仍不支持。
...
- 持久化 Permission/Delivery Ledger、`allowForSession` 和更多风险等级；当前只有会话内输入框写入的 `allowOnce/reject`；
- Agent 任务界面。
...
6. 加入 ActionProposal、权限和一个可撤销写入动作（代码已完成，待真实 Mac 验收与持久化）
