...
- Realtime Talk always exposes a bounded `write_focused_input` action tool; background Work draft, confirmation, status, and cancellation tools remain hidden unless final input transcription is configured
- Voice Agent startup locks one non-password Accessibility input target in local memory; an explicit write request creates a typed `ActionProposal`, validates the automatic reversible-write policy, and writes once without opening a confirmation surface
- Tool Call ID deduplication, target revision checks, and `ActionReceipt` distinguish success, failure, and an unverifiable paste; sending, submitting, publishing, purchasing, deleting, and permission changes remain unsupported
...
1. Validate the first automatic reversible action on a real Mac: TextEdit and one Web/Electron input, one write after an explicit request, `Cmd+Z`, duplicate Tool Call protection, target loss, and continued Talk during execution.
